...
func newDeployStopCmdWithDeps(ios *iostreams.IOStreams, deps *install.Deps) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "stop",
		Short: "Stop (pause) the Flintyst containers",
		Long: `Stop the Flintyst containers without removing them or their data.
...
func newDeployUninstallCmdWithDeps(ios *iostreams.IOStreams, deps *install.Deps) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "uninstall",
		Short: "Permanently delete the Flintyst deployment and all its data",
		Long: `Remove the Flintyst containers, volumes, and the deployment directory.
