...
// (without --raw). One `content` field per result, no Flintyst-internal jargon.
...
func newSearchCmd(ios *iostreams.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Search the Flintyst knowledge base and return ranked, cited documents.
...
search quality as the Flintyst chat interface.
