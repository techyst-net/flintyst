...
func newDeployUpgradeCmdWithDeps(ios *iostreams.IOStreams, deps *install.Deps) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "upgrade",
		Short: "Upgrade an existing Flintyst deployment to a newer version",
		Long: `Upgrade an existing Flintyst deployment to a newer version.
...
targets a stack that runs under a compose project name other than "onyx".`,
  ...
  onyx-cli deploy upgrade --prod --project danswer-stack --dir /opt/onyx --no-prompt`,
	...
	cmd.Flags().StringVar(&opts.Tag, "tag", "", "Image tag to upgrade to (default: the latest Flintyst release)")
