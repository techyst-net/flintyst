...
func newDeployInstallCmdWithDeps(ios *iostreams.IOStreams, deps *install.Deps) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "install",
		Short: "Install or restart a self-hosted Flintyst deployment",
		Long: `Install a self-hosted Flintyst deployment with docker compose, or restart /
...
every prompt, including Lite mode).`,
  ...
  onyx-cli deploy install --dry-run`,
	...
	cmd.Flags().BoolVar(&opts.Lite, "lite", false, "Deploy Flintyst Lite (no OpenSearch, Redis, or model servers)")
	cmd.Flags().BoolVar(&opts.IncludeCraft, "include-craft", false, "Enable Flintyst Craft (AI-powered web app building)")
	...
	cmd.Flags().StringVar(&opts.Tag, "tag", "", "Image tag to deploy (default: the latest Flintyst release)")
...
func newInstallOnyxCmd(ios *iostreams.IOStreams) *cobra.Command {
	...
	cmd.Short = "Install a self-hosted Flintyst deployment (alias for `deploy install`)"
	return cmd
}
