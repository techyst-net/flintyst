...
A CLI for querying enterprise knowledge from [Flintyst](https://github.com/onyx-dot-app/onyx). Includes an interactive chat TUI for humans and non-interactive commands for AI agents and scripts.
...
This prompts for your Flintyst server URL and personal access token (PAT), tests the connection, and saves config to `~/.config/onyx-cli/config.json` (or `$XDG_CONFIG_HOME/onyx-cli/config.json` if set). To reconfigure later, use the `/configure` command inside the TUI.
...
| `install-skill` | Agent / Script | Install the Flintyst CLI agent skill file |
| `experiments` | Agent / Script | List experimental features and their status |
| `serve` | Interactive | Serve the Flintyst TUI over SSH |
