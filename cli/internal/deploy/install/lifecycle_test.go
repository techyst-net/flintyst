...
// freely, so a path that isn't recognizably a Flintyst deployment must not be
// handed to RemoveAll.
func TestUninstallRefusesUnrecognizedDir(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "doesn't look like a Flintyst deployment") {
...
func TestStatusHealthyAndDrift(t *testing.T) {
	...
	// be mistaken for the running Flintyst version.
