...
func sampleModels() map[string]wizModel {
	...
	return map[string]wizModel{
		"question": {
			title:   "Flintyst Installer",
			version: "v0.1.0",
			stage:   StageConfigure,
			sel: &askSelectMsg{
				title: "How should Flintyst be deployed?",
				opts: []Option{
		...
		"confirm": {
			...
			sel: &askSelectMsg{
				title: "Enjoying Flintyst? ⭐ Star the repo on GitHub?",
...
func TestBoxesFillTerminalWidth(t *testing.T) {
	...
	for _, w := range sizes {
		...
		wiz.printTail(wizModel{width: w, card: []string{
			"🎉 Flintyst is ready  →  http://localhost:3000",
			"",
...
func TestFinishPrintsCardAfterQuit(t *testing.T) {
	...
	wiz.Finish("🎉 Flintyst is ready  →  http://localhost:3000")
...
func TestOptionHintsShareAColumn(t *testing.T) {
	...
	m := wizModel{sel: &askSelectMsg{title: "How should Flintyst be deployed?", opts: opts}}
