# Terraform Provider for Flintyst

Manages **Flintyst application configuration** declaratively via the Flintyst admin API: LLM
...
> Flintyst runs on (EKS, RDS, ...). This provider configures what runs *inside* a Flintyst
> deployment.
...
| `onyx_mcp_server` | MCP servers Flintyst connects to (`/admin/mcp`) | numeric id |
...
by an admin user). Create one in the Flintyst admin panel (*API Keys*) or via the API — pass the
...
and on Flintyst Cloud the tenant is embedded in the key itself.
...
- **`onyx_api_key.api_key`** is the key Flintyst mints, not one you supply. Terraform can only
  hand back a generated value through state. Treat the state file as holding it.
- **`onyx_mcp_server.auth_template_headers`** is computed — Flintyst writes the template itself
...
The counter only decides when an apply is *triggered*. Flintyst replaces all fields on update,
so the provider sends the secret on every apply it runs, whatever moved the plan.

### Two things to know

**`onyx_custom_tool` stops refreshing its headers.** Flintyst returns action headers in full
...
**Importing takes one extra apply.** Import reads what the server has, so a secret Flintyst
...
- **`onyx_settings` and `onyx_llm_provider_default` don't really delete.** Flintyst has no
...
- **`onyx_connector` does not own its access control.** `access_type` and `groups` are
  validated on write but stored on the cc-pair, so Terraform cannot refresh them. Flintyst also
  rewrites an unset `prune_freq` to 7 days on the first update, which the provider then
  keeps as the value of record.
- **`onyx_connector` does not set access control.** Flintyst applies it when a credential is
...
- **Deleting an agent leaves a tombstone.** Flintyst marks the row deleted instead of removing
...
- **A deleted agent answers 400, not 404.** The lookup raises a plain `ValueError`, which
  Flintyst renders as a bad request, so "gone" cannot be read off the status. The provider
...
- **`onyx_persona` does not own every field on an agent.** Attached folders and documents
  ...
  `search_start_date` is sent but never read back, because Flintyst returns it as a parsed
  timestamp that would not match a plain date. Avatar images are not managed at all.
- **`display_priority` is create-only on the upsert.** Flintyst reads it when an agent is
...
- **A roster change must not disturb those links, and how it avoids that depends on the
  change.** Flintyst's update endpoint replaces connector links along with members. A roster
  that only gains members therefore goes through the add-users endpoint instead, which
  takes members alone and lets Flintyst preserve the links itself, inside the transaction that
...
- **Flintyst refuses membership, rename and delete while a group is syncing**, and a newly
...
- **`onyx_user_group` permissions use Flintyst's wire tokens**, for example `manage:connectors`,
  not the enum names. Only toggleable permissions can be set; `basic`, `admin`,
  `craft_sandbox`, `manage:skills` and the implied read tokens are managed by Flintyst and are
...
- **Flintyst refuses a membership removal that would strand someone**, leaving them in no group
...
- **Which tools an MCP server exposes is not managed.** Flintyst only learns them by calling
...
- **An MCP server's `description` left out of the configuration is cleared, not kept.** Flintyst
...
- **An MCP server added from the admin panel but never configured imports with empty strings.**
  ...
  in one apply.
- **A configured `auth_template_headers` is never refreshed from Flintyst.** A header value may be
  a literal rather than a `{placeholder}`, and Flintyst masks those on the way out, so refreshing
  would store the mask and leave a difference that never settles. Flintyst's own template is read
...
- **`auth_performer = "PER_USER"` credentials belong to the identity that applied them.**
  Flintyst stores `admin_credentials` against the applying user rather than the server, so a
...
- **An MCP server's header template is never reset.** Flintyst keeps the stored template
...
- **`groups` and `users` on an MCP server are owned by the configuration.** Flintyst reads a
  ...
  admin panel.
- **An MCP server URL cannot point at the Flintyst host.** The SSRF guard refuses `localhost`
...
```bash
go build ./...        # build
go test ./...         # unit tests (no Flintyst needed)
```
...
Acceptance tests run real CRUD cycles against a live Flintyst deployment (they create and
...
Without `TF_ACC` these tests skip, so plain `go test ./...` stays green with no Flintyst
...
**Beat is required for the user group tests specifically.** Flintyst refuses to change or
...
connector's real `validate_connector_settings`, which reaches the source system; Flintyst
