...
// connector ids: Flintyst keeps the stored ones itself.
...
//
// Flintyst's update endpoint replaces connector links along with members, and
...
// one takes members alone, and Flintyst preserves the connector links itself,
...
// between is overwritten by the older list. Flintyst offers nothing narrower —
...
func (c *Client) SetUserGroupMembers(ctx context.Context, id int64, userIDs []string) (*UserGroup, error) {
	...
	// The whole roster goes out, not just the new names: Flintyst works out which
...
// Grants Flintyst manages itself are excluded, which matches what the provider
// is able to write.
...
// the stored set. Flintyst refuses any permission it does not let a group toggle.
...
// DeleteUserGroup asks Flintyst to delete a group. The row usually survives the
...
//
// Flintyst refuses a membership change, a rename and a delete while a group is
