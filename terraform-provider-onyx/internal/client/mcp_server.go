...
// MCPAuthTemplate is the header set Flintyst sends to the MCP server. Values may
...
type MCPServerWrite struct {
	...
	// so it is never refreshed from the server. APITokenChanged tells Flintyst to
...
// covers per-tool Craft policies, but Flintyst rejects a policy for a tool it has
