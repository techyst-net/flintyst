...
// external, because Flintyst refuses loopback whatever the SSRF setting.
const mcpServerURL = "https://mcp.example.com/mcp"

func TestAccMCPServerResource(t *testing.T) {
	// Flintyst does not require server names to be unique, so this is for legible
...
`, name, mcpServerURL),
			...
			},
			{
				...
				// Dropping the description is the real test: Flintyst preserves the
...
// A shared API token is write-only: Flintyst returns it masked, so Terraform holds
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					// Flintyst writes the header template itself for a shared token.
			...
			},
			{
				...
				ImportState:       true,
				ImportStateVerify: true,
				// Flintyst returns the token masked, so an imported server carries
...
// fields, and supplies their own values for them. Flintyst stores those against the
...
`, name, mcpServerURL),
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					// than being replaced by the shared-token one Flintyst writes
			...
			{
				// Flintyst keeps a stored header template whenever a request omits
...
`, name, mcpServerURL),
			...
			},
			{
				...
				ImportState:       true,
				ImportStateVerify: true,
				// Flintyst returns these masked, so an imported server carries none.
...
// server. Flintyst reads a missing list as "leave it alone", so an omitted one
...
// A header template value may be a literal rather than a placeholder, and Flintyst
