// Package provider implements the Terraform provider for Flintyst application
// configuration.
package provider
...
func (p *onyxProvider) Schema(_ context.Context, _ provider.SchemaRequest, resp *provider.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manage Flintyst application configuration (LLM providers, API keys, " +
			"workspace settings, ...) declaratively via the Flintyst admin API.",
		Attributes: map[string]schema.Attribute{
			"endpoint": schema.StringAttribute{
				Optional: true,
				MarkdownDescription: "Flintyst server origin, e.g. `https://cloud.onyx.app` or `http://localhost:3000`. " +
					"May also be set via the `ONYX_SERVER_URL` environment variable.",
			},
			"api_key": schema.StringAttribute{
				Optional:  true,
				Sensitive: true,
				MarkdownDescription: "Flintyst API key (`on_...`) in the seeded `Admin` group, or unrestricted " +
