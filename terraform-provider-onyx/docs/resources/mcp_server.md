...
- `api_token` (String, Sensitive) Shared API token, for `auth_type = "API_TOKEN"` with `auth_performer = "ADMIN"`. Onyx returns it masked, so Terraform never reads it back: the configured value is the only record, and an imported server has none. Prefer `api_token_wo`, which keeps the value out of state entirely; the two cannot be set together.
