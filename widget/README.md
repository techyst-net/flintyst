...
| ------------- | ------ | -------------------------------------------------------------------- |
| `backend-url` | string | Your Flintyst backend API URL (or set `VITE_WIDGET_BACKEND_URL` in .env) |
...
| `logo`             | string | Flintyst logo     | URL to custom logo image                 |
...
- Access to Flintyst backend API
...
│   └── assets/
│       └── logo.ts              # Default Flintyst logo (base64)
├── dist/                        # Build output
├── index.html
├── package.json
├── vite.config.ts
...
The widget communicates with these Flintyst backend endpoints:
