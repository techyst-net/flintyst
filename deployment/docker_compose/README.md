...
To set up Flintyst there are several options, Flintyst supports the following for deployment:
...
The script installs the Flintyst CLI (`onyx-cli`) and hands over to `onyx-cli deploy install`, which is
...
This provides a guided installation of Flintyst via Docker Compose. It will deploy the latest version of Flintyst
...
Flintyst maintains backwards compatibility across all minor versions following SemVer, so upgrading is
...
For example, to put Flintyst behind your own reverse proxy, stop nginx from publishing a host port and
...
IMAGE_TAG is the version of Flintyst to run. It is recommended to leave it as latest to get all updates with each redeployment.
