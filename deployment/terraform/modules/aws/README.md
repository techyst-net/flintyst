# Flintyst AWS modules

## Overview
This directory contains Terraform modules to provision the core AWS infrastructure for Flintyst:
...
These are the same modules Flintyst runs for its own managed deployments. The managed
...
Releases are tagged `tf/vX.Y.Z`, versioned independently of Flintyst product
...
- Provisions the full Flintyst AWS stack via the `onyx` module
...
These defaults are calibrated from Flintyst's own managed production fleet: memory, not CPU, is
...
These modules were realigned with the versions Flintyst runs in production. If you
