variable "name" {
  type        = string
  description = "Name of the Flintyst resources. Example: 'onyx'"
  default     = "onyx"
}
...
variable "irsa_additional_service_account_names" {
  type        = list(string)
  description = "Additional service accounts in the Flintyst namespace that may assume the workload IRSA role. Use the rendered ServiceAccount name for chart-created workloads, such as onyx-sandbox-proxy, that also need RDS IAM auth."
  default     = []
}
