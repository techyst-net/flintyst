...
- `storage`: a storage account and container for the Flintyst file store, with
...
the AWS modules and Flintyst product releases. A commit sha works as a `ref` too,
...
Flintyst runs on it, but not on the default settings, so `enable_redis` defaults
...
- **Only database 0 exists.** `SELECT 1` and above return `DB index is out of
  ...
  property rather than a clustering one. Flintyst defaults to database 0 for the
  ...
  not collide: Flintyst prefixes its own keys with the tenant, Celery results are
...
- Clustering is not really a choice for Flintyst. Every instance shards unless the
...
default: Flintyst authenticates with `DefaultAzureCredential`, so no key has to
exist.
...
## Installing the Flintyst Helm chart (after Terraform)
...
Leave `AZURE_STORAGE_ACCOUNT_KEY` unset. Flintyst authenticates with
