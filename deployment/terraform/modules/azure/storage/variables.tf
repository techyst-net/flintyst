...
variable "container_name" {
  type        = string
  description = "Blob container that holds the Flintyst file store. Maps to AZURE_FILE_STORE_CONTAINER_NAME."
...
# Flintyst authenticates to Blob with DefaultAzureCredential, so the account does
# not need shared keys. Leaving them off means a leaked key cannot exist.
variable "shared_access_key_enabled" {
  type        = bool
  description = "Allow authenticating with the account's shared keys. Off by default: Flintyst uses workload identity, and callers that need a key can set AZURE_STORAGE_ACCOUNT_KEY only after turning this on. Set storage_use_azuread = true on the azurerm provider so Terraform itself does not reach for a key either."
  default     = false
}
