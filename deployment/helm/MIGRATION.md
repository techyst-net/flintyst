...
Breaking changes between Flintyst Helm chart versions and what to do about them.
...
Flintyst; the api-server connected to it on `localhost:19071` (Vespa application
...
2. **Re-index is automatic.** Vespa data does not roundtrip directly
   ...
   manual action here; Flintyst connectors will reindex on their own once
   the api-server can reach the new endpoint (after the upgrade in
   step 5).
3. **Update your values** to point Flintyst at the external Vespa endpoint
...
The scheduled-tasks worker is only required if you use Flintyst's craft /
