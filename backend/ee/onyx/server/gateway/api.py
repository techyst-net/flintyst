...
def _authorize_gateway_request(http_request: Request, user: User) -> LLMFlow:
    ...
    if flow is None:
        raise OnyxError(
            ...
            "This credential is not authorized to use the Flintyst LLM gateway.",
        )
    return flow
...
def resolve_gateway_model(
    ...
    not_found_error = OnyxError(
        ...
        f"Model {requested_model!r} is not available through the Flintyst gateway.",
    )
...
def _anthropic_tools(
    ...
    for tool in raw_tools:
        ...
        if not name or input_schema is None:
            ...
            raise OnyxError(
                OnyxErrorCode.INVALID_INPUT,
                f"Tool {identifier!r} is not supported by the Flintyst gateway; "
