...
_NO_STORAGE_MESSAGE = (
    "The Flintyst gateway does not store responses; send the full "
...
def _build_upstream_request(
    ...
    if body.get("background"):
        raise OnyxError(
            OnyxErrorCode.NOT_IMPLEMENTED,
            "The Flintyst gateway does not support `background` responses.",
        )
    for tool in body.get("tools") or []:
        ...
        if tool_type == _TOOL_TYPE_MCP:
            ...
            raise OnyxError(
                ...
                "mcp tools are not supported by the Flintyst gateway.",
            )
        if tool_type == _TOOL_TYPE_FILE_SEARCH:
            ...
            raise OnyxError(
                OnyxErrorCode.INVALID_INPUT,
                "file_search tools are not supported by the Flintyst gateway.",
            )
        if tool_type == _TOOL_TYPE_CODE_INTERPRETER:
            ...
            if isinstance(container, str) or (
                ...
                raise OnyxError(
                    ...
                    "or uploaded files are not supported by the Flintyst gateway.",
                )
        ...
        for item in body["input"]:
            ...
            for part in content:
                if (
                    ...
                    raise OnyxError(
                        OnyxErrorCode.INVALID_INPUT,
                        "file_id references are not supported by the Flintyst "
    ...
    if body.get("prompt") is not None:
        ...
        raise OnyxError(
            OnyxErrorCode.INVALID_INPUT,
            "prompt template references are not supported by the Flintyst gateway.",
        )
