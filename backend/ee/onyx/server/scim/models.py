...
Admin API schemas are internal to Flintyst and used for SCIM token management.
"""
...
class ScimUserResource(BaseModel):
    ...
    id: str | None = None  # Flintyst's internal user ID, set on responses
...
class ScimGroupMember(BaseModel):
    ...
    or removing users from groups. ``value`` is the Flintyst user ID.
    """
...
# ---------------------------------------------------------------------------
# Admin API Schemas (Flintyst-internal, for SCIM token management)
# These are NOT part of the SCIM protocol. They power the Flintyst admin UI
