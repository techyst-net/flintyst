...
Flintyst uses Celery for asynchronous task processing. Worker apps live in
...
There are 4 main types of tests within Flintyst:
...
These should not assume any Flintyst/external services are available to be called.
...
These tests assume that all external dependencies of Flintyst are available and callable (e.g. Postgres, Redis,
MinIO/S3, OpenSearch are running + OpenAI can be called + any request to the internet is fine + etc.).

However, the actual Flintyst containers are not running and with these tests we call the function to test directly.
...
Standard integration tests. Every test in `backend/tests/integration` runs against a real Flintyst deployment. We cannot
...
These tests are an even more complete version of the Integration Tests mentioned above. Has all services of Flintyst
