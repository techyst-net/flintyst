...
from onyx.onyxbot.discord.constants import REGISTER_COMMAND, SYNC_CHANNELS_COMMAND
from onyx.onyxbot.discord.exceptions import RegistrationError, SyncChannelsError
...
async def handle_dm(message: discord.Message) -> None:
    ...
    dm_response = (
        ...
        f"[Flintyst Discord]({ONYX_DISCORD_URL}) for help!"
    )
...
async def handle_registration_command(
    ...
    try:
        ...
        await message.reply(
            ...
            "This server is now connected to Flintyst. "
            "I'll respond to messages based on your server and channel settings set in Flintyst."
        )
...
async def _register_guild(
    ...
    if existing_tenant is not None:
        ...
        raise RegistrationError(
            "This server is already registered.\n\nOnyxBot can only connect one Discord server to one Flintyst workspace."
        )
            ...
            with get_session_with_tenant(tenant_id=tenant_id) as db:
                ...
                if not config:
                    raise RegistrationError(
                        ...
                        "Please generate a new one from the Flintyst admin panel."
                    )
                ...
                if config.guild_id is not None:
                    raise RegistrationError(
                        ...
                        "Please generate a new key from the Flintyst admin panel."
                    )
    ...
    try:
        ...
        def _get_guild_config_id() -> int | None:
            with get_session_with_tenant(tenant_id=tenant_id) as db:
                if not message.guild:
                    raise SyncChannelsError(
                        "Server not found. This shouldn't happen. Please contact Flintyst support."
                    )
        ...
        if not guild_config_id:
            raise SyncChannelsError(
                "Server config not found. This shouldn't happen. Please contact Flintyst support."
            )
        ...
        await message.reply(
            ...
            "New channels are disabled by default. Enable them in the Flintyst admin panel."
        )
