...
def _check_bot_config_api_access() -> None:
    ...
    - On Cloud (managed by Flintyst)
    - When DISCORD_BOT_TOKEN env var is set (managed via env)
    """
    if MULTI_TENANT:
        raise OnyxError(
            OnyxErrorCode.INSUFFICIENT_PERMISSIONS,
            "Discord bot configuration is managed by Flintyst on Cloud.",
        )
...
def delete_service_api_key_endpoint(
    ...
    Discord bot to authenticate with the Flintyst API. The key is also automatically
    deleted when:
