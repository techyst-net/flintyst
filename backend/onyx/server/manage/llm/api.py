...
def _get_ollama_available_model_names(api_base: str) -> set[str]:
    ...
    URL, so a server Flintyst can't route to is a client misconfiguration.
    """
    ...
    try:
        ...
        raise OnyxError(
            ...
            f"is correct and reachable from Flintyst ({type(e).__name__}).",
        )
    except Exception as e:
        raise OnyxError(
            OnyxErrorCode.BAD_GATEWAY,
...
def _get_openai_compatible_models_response(
    ...
    try:
        ...
        raise OnyxError(
            ...
            f"correct and reachable from Flintyst ({type(e).__name__}).",
        )
    except ValueError as e:
        logger.warning(
