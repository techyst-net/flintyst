"""Resources that expose metadata for the Flintyst MCP server."""
...
@mcp_server.resource(
    ...
    description=(
        "Enumerate the user's document sources that are currently indexed in Flintyst."
...
async def indexed_sources_resource() -> str:
    ...
    logger.info(
        "Flintyst MCP Server: indexed_sources resource returning %s entries",
        len(sources),
    )
