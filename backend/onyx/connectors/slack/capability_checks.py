    ...
    def run(self, context: CapabilityCheckContext) -> None:
        ...
        if "channels:join" not in granted_scopes:
            raise InsufficientPermissionsError(
                "The bot token lacks the `channels:join` scope, so Flintyst cannot "
    ...
    def run(self, context: CapabilityCheckContext) -> None:
        ...
        try:
            ...
            _raise_for_slack_api_error(
                ...
                "permission sync requires to map Slack members to Flintyst users "
                "by email.",
            )
        human_members = [
            member
        ...
        if not any(member.get("profile", {}).get("email") for member in human_members):
            raise InsufficientPermissionsError(
                ...
                "permission sync cannot map Slack members to Flintyst users. This "
...
class _GridPublicChannelScopingCheck(CapabilityCheck):
    ...
    every public channel visible org-wide in Flintyst.
    """
    ...
    def run(self, context: CapabilityCheckContext) -> None:
        ...
        try:
            ...
            _raise_for_slack_api_error(
                ...
                "channel visible org-wide in Flintyst (over-sharing).",
            )
