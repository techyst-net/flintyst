    ...
    def resolve(self, request: http.Request, ctx: InjectionContext) -> dict[str, str]:
        ...
        try:
            ...
            raise CredentialUnavailableError(
                str(e),
                sandbox_detail=(
                    "Multiple MCP servers in Flintyst are configured with the same "
        ...
        if target is None:
            ...
            raise CredentialUnavailableError(
                ...
                sandbox_detail=(
                    "This host belongs to an MCP server configured in Flintyst, but "
...
def _fix_instruction(admin_managed: bool, *, reconnect: bool) -> str:
    ...
    if admin_managed:
        return f"Ask a workspace admin to {verb} it on the MCP actions page in Flintyst."
