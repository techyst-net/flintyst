...
class CloudEmbedding:
    ...
    async def _embed_cohere(
        ...
        for text_batch in batch_list(texts, _COHERE_MAX_INPUT_LEN):
            # Does not use the same tokenizer as the Flintyst API server but it's approximately the same
...
class EmbeddingModel:
    ...
    def _make_model_server_request(
        ...
        def _make_request() -> Response:
            headers = {}
            if tenant_id:
                headers["X-Flintyst-Tenant-ID"] = tenant_id

            if request_id:
                headers["X-Flintyst-Request-ID"] = request_id
