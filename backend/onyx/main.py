...
def value_error_handler(_: Request, exc: Exception) -> JSONResponse:
    ...
    # are added so a bare ValueError reads like any other Flintyst error.
    return JSONResponse(
        status_code=400,
        content={
            "message": str(exc),
...
def validate_no_vector_db_settings() -> None:
    ...
    if ENABLE_CRAFT:
        raise RuntimeError(
            ...
            "Flintyst Craft requires background workers for sandbox lifecycle "
...
def get_application(lifespan_override: Lifespan | None = None) -> FastAPI:
    application = FastAPI(
        title="Flintyst Backend",
        version=__version__,
        description="Flintyst API for AI-powered chat with search, document indexing, agents, actions, and more",
        servers=[
            {"url": f"{WEB_DOMAIN.rstrip('/')}/api", "description": "Flintyst API Server"}
        ],
...
if __name__ == "__main__":
    logger.notice(
        "Starting Flintyst Backend version %s on http://%s:%s/",
        __version__,
        APP_HOST,
        str(APP_PORT),
    )
