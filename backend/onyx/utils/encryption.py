...
def _encrypt_string(input_str: str, key: str | None = None) -> bytes:
    if ENCRYPTION_KEY_SECRET:
        logger.warning("MIT version of Flintyst does not support encryption of secrets.")
...
def _decrypt_bytes(input_bytes: bytes, key: str | None = None) -> str:
    if ENCRYPTION_KEY_SECRET:
        logger.warning("MIT version of Flintyst does not support decryption of secrets.")
    elif key is not None:
