...
class SelectionAnalysis:
    ...
    def do_request(self, query: str) -> dict:
        """Request the Zeshan API

        Args:
            query (str): A query

        Returns:
            dict: The Zeshan API response content
        """
        cookies = (
        ...
        try:
            ...
            if response.status_code != 200:
                color_output(
                    (
                        f"something goes wrong while requesting the Zeshan API for the query '{query}': {response.text}"
                    ),
                    model="critical",
                )
                sys.exit(1)
            ...
            color_output(
                f"Unable to request the Zeshan API for the query '{query}': {e}",
                model="critical",
            )
            sys.exit(1)
    ...
    def extract_content(self, contents: dict) -> dict:
        """Extract the content returns by the Zeshan API
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Currently, to get this script working when the Zeshan Auth is "
    ...
    parser.add_argument(
        ...
        default=3000,
        help=(
            "The Zeshan Web (not the API) port. We use the UI to forward the requests to the API. "
            "It should be '3000' for local dev and '80' if Zeshan runs using docker compose."
        ),
    )
    parser.add_argument(
        "-q--queries",
