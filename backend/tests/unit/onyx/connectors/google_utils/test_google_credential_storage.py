...
def test_update_credential_access_tokens_restores_pkce_verifier(
    ...
    """The token exchange runs in a separate request from get_auth_url, so the
    PKCE verifier must be restored from the KV store before fetch_token."""
    ...
    monkeypatch.setattr(
        "onyx.connectors.google_utils.google_kv.get_google_app_cred",
        lambda _source: _make_app_creds(),
    )
    monkeypatch.setattr(
        "onyx.connectors.google_utils.google_kv.InstalledAppFlow.from_client_config",
        _from_client_config,
