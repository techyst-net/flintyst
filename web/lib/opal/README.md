...
Flintyst's TypeScript component library and design system.
...
`colors.css` or import a copy from Flintyst. The preset references them but does not define them —
...
## Local development (inside the Flintyst repo)
...
so Flintyst's web app keeps building.

The package is consumed by `web/` as a workspace via `web/package.json`'s `"@onyx-ai/opal":
"./lib/opal"`. During Flintyst development, `web/` resolves Opal source through the `@opal/*`
...
products Flintyst integrates with. Marks remain the property of their
respective owners; Flintyst claims no trademark over them. See
