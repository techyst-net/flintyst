...
6. [06-unified-chat-surface.md](06-unified-chat-surface.md) — post-PR 6 refactor (`unify-chat-input`): collapse the forked `ChatConversation` + `ProjectView` chrome into **one persistent `ChatSurface`** in `(app)/_layout.tsx` so new/chat/project morph in place (no screen swap), composer declared once. Includes the `renderRouter` spike result (PASS) and a grill-on checklist.
