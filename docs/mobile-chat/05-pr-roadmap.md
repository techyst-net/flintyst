...
| 2 | `feat(mobile): native chat data layer` | ~500 | PR 1 | Mobile-native NDJSON parser + core packet/chat/file **contracts** + message tree + `processRawChatHistory`; jest unit-tested. **Web untouched; nothing shared** (see PR 2 Decision). |
...
## PR 2 — Mobile-native chat data layer
...
- **Goal:** A typed, unit-tested mobile-native chat data layer — no UI, no streaming yet.
- **Scope (in):** `mobile/src/chat/ndjson.ts` — a pure NDJSON line buffer (partial-line carry, brace-recovery fallback, trailing flush), mirroring web's `handleSSEStream` parsing core; `mobile/src/chat/contracts/` — minimal packet types (`MESSAGE_START/DELTA/END`, `STOP`+`StopReason`, `SECTION_END`, `ERROR`/`PacketError`, `message_id_info`, `chat_heartbeat`, `Placement`/`Packet` wrapper; `MessageStart.final_documents` typed `unknown[] | null` — no `OnyxDocument`), `Message`/`ChatState`/`BackendMessage` (minimal), `FileDescriptor`/`ChatFileType`, create-session + send-message request/response bodies; `mobile/src/chat/messageTree.ts` — `upsertMessages`/`getLatestMessageChain`/`getMessageByMessageId`/builders/`SYSTEM_NODE_ID` (port web's logic against the minimal `Message`); `mobile/src/chat/chatHistory.ts` — `processRawChatHistory` (minimal: structural tree only, strips rich doc/citation/research fields). Jest unit tests for parser + tree + history.
- **Out of scope:** Streaming transport, store, hooks, any RN UI (PR 3). Rich packet types (their PR 9 phase). Any change to web or to `@onyx-ai/shared`.
- **Files:** `mobile/src/chat/ndjson.ts` (new), `mobile/src/chat/contracts/*` (new), `mobile/src/chat/messageTree.ts` (new), `mobile/src/chat/chatHistory.ts` (new), `mobile/src/chat/*.test.ts` (new). **Web + `@onyx-ai/shared` untouched.**
- **Est. size:** ~500 LOC (incl. tests).
- **Depends on:** PR 1 (logical; pure mobile TS — needs no shell, but sequences after the `(app)` group exists).
- **Feature-flag state:** N/A.
- **Tests on merge:** Jest unit (mobile runner) — NDJSON buffering (partial lines, brace-recovery, heartbeat passthrough, trailing flush); `upsertMessages` (system-node creation, latest-child rules, multi-add); `getLatestMessageChain`; `processRawChatHistory` (parent/children wiring, error-typed nodes). These protect the (mobile-owned) parser + tree + history.
- **Before you start (grill on):** The exact minimal packet `type` set. The minimal `Message`/`BackendMessage` field set (avoid dragging `OnyxDocument`/multi-model). Whether the tree is a verbatim port of web's `messageTree.ts` or trimmed to the ~4 functions core needs. Whether the parser is a standalone pure buffer or inlined into PR 3's `stream.ts` (a separate buffer is more unit-testable). Where the mobile chat layer lives (`mobile/src/chat/`).
- **Drift checkpoint:** Note in code that `ndjson.ts` + `messageTree.ts` + `processRawChatHistory` mirror web's `streamingUtils.ts` + `messageTree.ts` + `lib.tsx` — keep the NDJSON framing + structural tree semantics in sync if the backend protocol changes. (This is the duplication cost we accepted over a shared seam.)
...
- **Files:** `mobile/src/chat/fileDescriptors.ts` (new, mobile-native), `components/chat/AttachmentChips.tsx` (new), `InputBar.tsx` (modified), `useChatController.ts` (modified: attach descriptors). Web `fileUtils.ts` **untouched**.
- **Est. size:** ~550 LOC.
- **Depends on:** PR 7.
- **Feature-flag state:** N/A.
